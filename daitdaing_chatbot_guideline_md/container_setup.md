...
`pgvector/pgvector:pg16` 컨테이너에 Itdaing Seed 데이터를 불러와 LangGraph 챗봇이 즉시 사용할 수 있는 임베딩 컬렉션을 구축하는 절차를 설명합니다. 모든 명령은 macOS zsh 기준이지만 Windows PowerShell에서도 동일하게 동작합니다.
...
1. 컨테이너에 `/seed` 디렉터리를 만든 후 JSON을 복사합니다.
   ...
   docker cp data/itdaing_seed.json pgvector-container:/seed/itdaing_seed.json
   ```
2. `seed_snapshots` 테이블 생성 및 JSON 저장 (선택).
   ...
   SELECT pg_read_file('/seed/itdaing_seed.json')::jsonb, '/seed/itdaing_seed.json'
...
Seed JSON을 파싱하고 OpenAI 임베딩을 생성하는 작업은 `scripts/load_pgvector.py`가 담당합니다.
```bash
python scripts/load_pgvector.py --reset --include-prompts
```
- 필수 환경 변수: `PGVECTOR_CONNECTION`, `OPENAI_API_KEY`, `VECTOR_COLLECTION`
- `--reset`: 기존 컬렉션 삭제 후 재생성
- `--include-prompts`: `data/test_prompts.json`의 테스트 문장까지 임베딩해 품질 점검 시 활용
